...
if [[ $# -ge 1 ]]; then
    ...
    read -r -p "请输入模块（pe/cmp/input_delayer/output_delayer/sa/delayer_sa/accumulator/accumulator_pipeline/fsa_core/fsa_core_execute/fsa_core_full/sram）：" MODULE
fi
...
case "$MODULE" in
    pe|cmp|input_delayer|output_delayer|sa|delayer_sa|accumulator|accumulator_pipeline|fsa_core|fsa_core_execute|fsa_core_full)
        ;;
    sram|banked_sram)
    ...
    *)
        ...
        echo "用法：$0 pe|cmp|input_delayer|output_delayer|sa|delayer_sa|accumulator|accumulator_pipeline|fsa_core|fsa_core_execute|fsa_core_full|sram" >&2
        exit 1
        ;;
esac
