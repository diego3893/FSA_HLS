...
if [[ $# -ge 1 ]]; then
    ...
    read -r -p "请输入模块（pe/cmp/input_delayer/output_delayer/sa/delayer_sa/accumulator/fsa_core/sram）：" MODULE
fi
...
case "$MODULE" in
    ...
    *)
        ...
        echo "用法：$0 pe|cmp|input_delayer|output_delayer|sa|delayer_sa|accumulator|fsa_core|sram" >&2
        exit 1
        ;;
esac
