...
void accumulator_step(const AccumulatorState& current,
    ...
    for(int col=0; col<SA_COLS; ++col){
        const acc_t in_a = exp_s1 ? io.sa_in[(std::size_t)col]
                                  : current.scale[col];
        const acc_t in_b = exp_s1 ? attentionScale(SA_ROWS)
        ...
        if(current.reciprocal_busy[col]){
            if(current.reciprocal_counter[col] <= 1){ // 最后一拍计算结束，保存并标记有效
                ...
                next.reciprocal_counter[col] = current.reciprocal_counter[col]-1;
            }
        }else if(valid && reciprocal_cmd){
            next.reciprocal_busy[col] = true;
            next.reciprocal_counter[col] = (reciprocal_counter_t)(reciprocalLatency-1);
